              ...
              <nav className="p-4 space-y-2">
                {navigation.map((item) => {
                  ...
                  const canView = (() => {
                    switch (item.name) {
                      case 'Dashboard':
                        return !!user && hasRole([Roles.ORGADMIN, Roles.MANAGER, Roles.STAFF_EMPLOYEE, Roles.VIEWER])
                      case 'Clients':
                      case 'Leads':
                      case 'Deals':
                        return hasRole([Roles.ORGADMIN, Roles.MANAGER])
                      case 'Projects':
                        return hasRole([Roles.ORGADMIN, Roles.MANAGER, Roles.STAFF_EMPLOYEE])
                      case 'Tasks':
                        return hasRole([Roles.ORGADMIN, Roles.MANAGER, Roles.STAFF_EMPLOYEE])
                      case 'Reports':
                        return hasRole([Roles.ORGADMIN, Roles.MANAGER, Roles.VIEWER])
                      case 'Settings':
                        return hasRole(Roles.ORGADMIN)
                      default:
                        return true
                    }
                  })()
                      ...
                      <div className="p-4 border-b border-neu-300">
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1">
                            <p className="font-brutal font-bold text-black">{user?.firstName} {user?.lastName}</p>
